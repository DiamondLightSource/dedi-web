...
export default class QSpace {
    ...
    constructor(detProps: DetectorProperties, diffexp: DiffractionCrystalEnvironment, qScale: number) {
    ...
    qFromPixelPosition(x: number, y: number) {
        const q = new Vector3();
        q.set(-this.detProps.pixelSize.height * (x), -this.detProps.pixelSize.width * (y), 0);
